...
<div class="btn" align="center"><button onclick="window.print()" style="padding:3px 15px; font-weight:bold">Print</button><button onclick="window.location='pathology.php?active=pathology'" style="padding:3px 15px; font-weight:bold">Back</button></div>
<?php
	...
	$check_invoice = $mysqli->query("select * from path_result_pages where idNo = '$_POST[idNo]'");
    ...
    <?php
		...
		$mysqli->query("insert into path_result_pages (idNo,pages,resultDate,resultTime) values('$_POST[idNo]', '$_POST[pageCombination]', '$date', '$time')") or die($mysqli->error);
		...
		foreach ($postValue as $key => $value) {
				...
				foreach ($value as $key2 => $value2) {
					if($value2 <> ''){
						...
						$mysqli->query("insert into report_output (reportContentId,reportId,idNo,value,pageSl) values( '$key', '$key2', '$_POST[idNo]', '$value2', '$pageSl[$i]')");
					}
				}
				$i++;
		}
